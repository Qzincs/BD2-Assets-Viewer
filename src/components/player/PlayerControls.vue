...
import { ChevronUp, LockKeyhole, LockKeyholeOpen, Pause, Play, RotateCcw } from '@lucide/vue'
...
const showTimelinePreview = computed(() => {
    return timelineHovering.value
        && !timelineDragging.value
        && hoverProgress.value > progress.value
})

const selectedAnimationLabel = computed(() => {
    return props.animationOptions.find((option) => option.id === selectedAnimationId.value)?.label
...
function focusRelativeAnimation(offset: number) {
    ...
    const currentIndex = focusedAnimationIndex.value >= 0
        ? focusedAnimationIndex.value
        : 0
    const nextIndex = (
        currentIndex
        + offset
    ...
    focusedAnimationId.value = filteredAnimationOptions.value[nextIndex]?.id ?? focusedAnimationId.value
    scrollFocusedAnimationIntoView()
}
...
<template>
    <div class="absolute left-0 right-0 bottom-0 border-t border-white/10 bg-black/50 text-white backdrop-blur-md">
        <button ref="timelineRef" class="group absolute -top-1 left-0 z-10 h-3 w-full cursor-pointer touch-none"
            type="button" aria-label="Timeline" @pointerenter="timelineHovering = true"
            @pointermove="updateTimelineProgress" @pointerleave="leaveTimeline"
            ...
            <span
                ...
                <span v-if="showTimelinePreview" class="absolute h-full bg-white/35" :style="{
                    left: `${progress * 100}%`,
                    width: `${(hoverProgress - progress) * 100}%`,
                }"></span>
                <span class="absolute top-0 left-0 h-full bg-white/75 transition-colors group-hover:bg-white"
                    :style="{ width: `${progress * 100}%` }"></span>
            </span>
            <span v-if="timelineDragging"
        ...
        <div class="flex h-14 items-center justify-between px-5 pt-2">
            <button
                ...
                type="button" aria-label="Play or pause" @click="togglePlayback">
            ...
            <div class="flex items-center gap-2">
                <div ref="animationMenuRef" class="relative">
                    <button
                        ...
                        type="button" aria-haspopup="listbox" :aria-expanded="animationMenuOpen"
                    ...
                    <div v-if="animationMenuOpen"
                        ...
                        role="listbox" :aria-activedescendant="`animation-option-${focusedAnimationId}`">
                        <div class="sticky top-0 z-10 border-b border-white/10 bg-black/80 p-2 backdrop-blur-md">
                            <input ref="animationSearchInputRef" v-model="animationSearchQuery"
                                class="h-8 w-full rounded-md bg-white/10 px-2 text-sm text-white outline-none placeholder:text-white/35 focus:bg-white/15"
                                type="search" placeholder="Search animations" aria-label="Search animations"
                                @keydown.stop="handleAnimationMenuKeydown" />
                        </div>

                        <button v-for="animationOption in filteredAnimationOptions" :key="animationOption.id"
                            :ref="(element) => setAnimationOptionElement(animationOption.id, element)"
                            :id="`animation-option-${animationOption.id}`"
                            class="w-full truncate px-3 py-2 text-left text-sm transition hover:bg-white/10" :class="{
                            ...
                            }" type="button" role="option" :aria-selected="animationOption.id === selectedAnimationId"
                            @mouseenter="focusedAnimationId = animationOption.id"
                            @click="selectAnimation(animationOption)">
                            {{ animationOption.label }}
                        </button>

                        <div v-if="filteredAnimationOptions.length === 0"
                ...
                <div ref="speedMenuRef" class="relative">
                    <button
                        ...
                        type="button" aria-haspopup="listbox" :aria-expanded="speedMenuOpen" aria-label="Playback speed"
                        @click="speedMenuOpen = !speedMenuOpen">
                        {{ playbackSpeed.toFixed(2) }}x
                    </button>

                    <div v-if="speedMenuOpen"
                        ...
                        role="listbox">
                        <button v-for="speedOption in playbackSpeedOptions" :key="speedOption"
                            ...
                            type="button" role="option" :aria-selected="speedOption === playbackSpeed"
                ...
                <button
                    class="flex h-9 w-9 items-center justify-center rounded-full text-xl text-white/75 transition hover:bg-white/10 hover:text-white active:scale-95"
                    type="button" aria-label="Reset view" @click="emit('resetView')">
