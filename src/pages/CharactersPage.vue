...
const filteredCostumes = computed(() => {
    return props.displayCostumes.filter(c => {
        return (!activeFilters.element || c.element === activeFilters.element)
...
const groupedCostumes = computed(() => {
    return rarityOrder
        .map((rarity) => ({
            rarity,
            costumes: filteredCostumes.value.filter(
                (costume) => costume.rarity === rarity,
            ),
        }))
...
<template>
    <section class="flex-1 p-4">
        <CostumeFilterBar :groups="groups" :active-filters="activeFilters"
        ...
        <template v-else>
            <section v-for="group in groupedCostumes" :key="group.rarity" class="mb-6">
                ...
                <div class="mt-4 grid gap-4 [grid-template-columns:repeat(auto-fill,minmax(var(--grid-size),1fr))]"
                    style="--grid-size: 120px;">
                    <CostumeCard v-for="costume in group.costumes" :key="costume.id" :id="costume.id"
                        :character-name="costume.characterName" :costume-name="costume.costumeName"
                        :element="costume.element" @select="openCostume" />
                </div>
            </section>
        </template>
